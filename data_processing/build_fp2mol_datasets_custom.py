...
if False:
    ...
    canopus_split = pd.read_csv(
        "../data/canopus/splits/canopus_hplus_100_0.tsv", sep="\t"
    )
    ...
    for i in tqdm(
        range(len(canopus_labels)),
        desc="Converting CANOPUS SMILES to InChI",
        leave=False,
    ):
...
for i in tqdm(
    range(len(neims_labels)), desc="Converting NEIMS SMILES to InChI", leave=False
):
...
for i in tqdm(
    range(len(franklin_labels)), desc="Converting franklin SMILES to InChI", leave=False
):
...
franklin_train_df.to_csv(
    "../data/franklin/franklin/preprocessed/franklin_train.csv", index=False
)

franklin_test_df = pd.DataFrame(franklin_test_inchis, columns=["inchi"])
franklin_test_df.to_csv(
    "../data/franklin/franklin/preprocessed/franklin_test.csv", index=False
)

franklin_val_df = pd.DataFrame(franklin_val_inchis, columns=["inchi"])
franklin_val_df.to_csv(
    "../data/franklin/franklin/preprocessed/franklin_val.csv", index=False
)
