...
class NodeEdgeBlock(nn.Module):
    ...
    def __init__(self, dx, de, dy, n_head, device=None, dtype=None, **kwargs) -> None:
        ...
        self.y_out = nn.Sequential(
            nn.Linear(dy, dy, **kw), nn.ReLU(), nn.Linear(dy, dy, **kw)
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mlp_in_X = nn.Sequential(
            nn.Linear(input_dims["X"], hidden_mlp_dims["X"], **kw),
            act_fn_in,
            nn.Linear(hidden_mlp_dims["X"], hidden_dims["dx"], **kw),
            act_fn_in,
        )

        self.mlp_in_E = nn.Sequential(
            nn.Linear(input_dims["E"], hidden_mlp_dims["E"], **kw),
            act_fn_in,
            nn.Linear(hidden_mlp_dims["E"], hidden_dims["de"], **kw),
            act_fn_in,
        )

        self.mlp_in_y = nn.Sequential(
            nn.Linear(input_dims["y"], hidden_mlp_dims["y"], **kw),
            act_fn_in,
            nn.Linear(hidden_mlp_dims["y"], hidden_dims["dy"], **kw),
            act_fn_in,
        )
        ...
        self.mlp_out_X = nn.Sequential(
            nn.Linear(hidden_dims["dx"], hidden_mlp_dims["X"], **kw),
            act_fn_out,
            nn.Linear(hidden_mlp_dims["X"], output_dims["X"], **kw),
        )

        self.mlp_out_E = nn.Sequential(
            nn.Linear(hidden_dims["de"], hidden_mlp_dims["E"], **kw),
            act_fn_out,
            nn.Linear(hidden_mlp_dims["E"], output_dims["E"], **kw),
        )

        self.mlp_out_y = nn.Sequential(
            nn.Linear(hidden_dims["dy"], hidden_mlp_dims["y"], **kw),
            act_fn_out,
