...
def apply_encoder_finetuning(model, strategy):    
    if strategy is None:
        ...
        for name, param in model.encoder.named_parameters():
            layer = name.split('.')[1]
            if layer not in ['0', '1']:  # Layer 0 = transformer, 1 = fragment predictor
                param.requires_grad = False
    else:
...
def main(cfg: DictConfig):
    ...
    if dataset_config["name"] not in ("canopus", "msg", "neims", "neims_tms", 
                                      "msg_neims", "mixed_augment_test", "mixed_augment"):
