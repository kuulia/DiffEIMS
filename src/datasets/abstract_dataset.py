...
class CustomLightningDataset(LightningDataset):
    ...
    def __init__(self, cfg, datasets: dict, **kwargs):
        ...
        try:
            ...
            super().__init__(
                train_dataset=None, val_dataset=None, test_dataset=datasets["test"]
            )
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        )  # is instance of IterableDataset: e.g. to avoid streaming/lazy loading incompatibility
        shuffle &= (
            self.kwargs.get("sampler", None) is None
        )  # a custom sampler is provided
        shuffle &= (
            self.kwargs.get("batch_sampler", None) is None
        )  # a custom batch sampler is provided
...
ATOM_DECODER = ("C", "N", "S", "O", "F", "Cl", "H", "P", "B", "Br", "I", "Si")
